...
class DemoVR extends Demo {
    constructor() {
        ...
        this._loadControllers = this._loadControllers.bind(this);
    ...
    _addVREventListeners() {
        ...
        window.addEventListener('gamepadconnected', this._loadControllers);

        window.addEventListener('gamepaddisconnected', this._loadControllers);
    }
        ...
        for (var i = 0; i < this._gamepads.length; ++i) {
            ...
            for (var j = 0; j < gamepad.buttons.length; ++j) {
                if (gamepad.buttons[j].pressed) {
                    console.log("Manette " + i + ", bouton " + j + ", value : " + gamepad.buttons[j].value);
                    this._box.material.color.setHex(Math.random() * 0xFFFFFF << 0);
                    break;
                }
            }
            for (var j = 0; j < gamepad.axes.length; ++j) {
                if ((gamepad.axes[j] > 0.1) || (gamepad.axes[j] < -0.1)) {
                    console.log("Manette " + i + ", axe " + j + ", value : " + gamepad.axes[j]);
                    this._box.material.color.setHex(Math.random() * 0xFFFFFF << 0);
                    break;
                }
            }
